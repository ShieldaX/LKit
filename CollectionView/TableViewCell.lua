...
function TableViewCell:initialize(opt)
  ...
  -- main image view
  if opt.image then
  --if opt.image and isImageExist(opt.image) then
    local image = display.newImageRect(bounds, opt.image, contentWidth, contentHeight)
    image.x = contentWidth*.5
    image.y = contentHeight*.5
    self.image = image
  end
...
function  TableViewCell:setImage(imagePath)  
  local image = self.image
  if image then image:removeSelf() end
  local contentHeight = self.background.contentHeight
  local contentWidth = self.background.contentWidth
  image = display.newImageRect(self.bounds, imagePath, contentWidth, contentHeight)
  image.x = contentWidth*.5
  image.y = contentHeight*.5
  self.image = image
end
