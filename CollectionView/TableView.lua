    ...
    if not availableCells[name] then -- if cell is not available
      ...
      if not cell then -- create new table cell instance
        cell = TableViewCell {
          ...
          image = dataSource:imageForRowAtIndexPath(indexPath),
          y = offset,
          height = dataSource:heightForRowAtIndexPath(indexPath),
          width = dataSource.columnWidth,
        ...
        cell:setImage(dataSource:imageForRowAtIndexPath(indexPath))
